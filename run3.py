...
def save_output_file_with_fixed_cost(costs, fixed_costs, allocation, supply, demand, method_name, instance_name):
    ...
    output_dir = "Lab_FCR_solve_DIST"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    file_name = f"{instance_name}_{method_name}_DIST.txt"
...
def solve_instance_with_fixed_cost(costs, supply, demand, fixed_costs, method_name, instance_name):
    ...
    if method_name == "rm":  
        for i in range(len(supply)):
            while supply_left[i] > 0 and np.sum(demand_left) > 0:
                ...
                for j in range(len(demand)):
                    if demand_left[j] > 0:
                        cost = costs[i][j] + (fixed_costs[i][j] / demand_left[j]) 
                        if cost < min_cost:
                            min_cost = cost
                            min_j = j
                if min_j == -1:
        ...
        while np.sum(supply_left) > 0 and np.sum(demand_left) > 0:
            ...
            for i in range(len(supply)):
                if supply_left[i] > 0: 
                    row_costs = [(costs[i][j] + (fixed_costs[i][j] / max(demand_left[j], 1)), j) for j in range(len(demand)) if demand_left[j] > 0]
                    if len(row_costs) > 1:
            ...
            for j in range(len(demand)):
                if demand_left[j] > 0:  
                    col_costs = [(costs[i][j] + (fixed_costs[i][j] / max(demand_left[j], 1)), i) for i in range(len(supply)) if supply_left[i] > 0]
                    if len(col_costs) > 1:
            ...
            if axis == 'row':
                min_cost, min_j = min((costs[idx][j] + (fixed_costs[idx][j] / max(demand_left[j], 1)), j) for j in range(len(demand)) if demand_left[j] > 0)
                ...
                min_cost, min_i = min((costs[i][idx] + (fixed_costs[i][idx] / max(demand_left[idx], 1)), i) for i in range(len(supply)) if supply_left[i] > 0)
...
def process_all_instances_with_fixed_cost():
    ...
    for file_name in os.listdir("Lab_FCR_instances"):
        if file_name.endswith(".dat"):
            ...
            with open(file_path, "r") as f:
                ...
                Cjk = np.array(Cjk_numbers).reshape((d, r))  # Reshape into `d x r` matrix
                ...
                Fjk = np.array(Fjk_numbers).reshape((d, r))  # Reshape into `d x r` matrix
    ...
    for method, result_data in results.items():
        if result_data:
            ...
            output_file = f"Lab_FCR_solved_{method.upper()}_DIST.xlsx"
