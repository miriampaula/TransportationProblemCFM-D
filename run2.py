...
def save_output_file_with_fixed_cost(costs, fixed_costs, allocation, supply, demand, method_name, instance_name):
    ...
    output_dir = "Lab_FCR_solved_TCC"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    file_name = f"{instance_name}_{method_name}_TCC.txt"
...
def solve_instance_with_fixed_cost(costs, supply, demand, fixed_costs, method_name, instance_name):
    ...
    if method_name == "rm":  
        for i in range(len(supply)):
            while supply_left[i] > 0 and np.sum(demand_left) > 0:
                ...
                for j in range(len(demand)):
                    if demand_left[j] > 0:
                        cost = (costs[i][j] * demand_left[j]) + fixed_costs[i][j]  
                        if cost < min_cost:
                            min_cost = cost
                            min_j = j
                if min_j == -1:
        ...
        while np.sum(supply_left) > 0 and np.sum(demand_left) > 0:
            ...
            for i in range(len(supply)):
                if supply_left[i] > 0: 
                    row_costs = [(costs[i][j] * min(demand_left[j], supply_left[i]) + fixed_costs[i][j], j) for j in range(len(demand)) if demand_left[j] > 0]
                    if len(row_costs) > 1:
            ...
            for j in range(len(demand)):
                if demand_left[j] > 0:  
                    col_costs = [(costs[i][j] * min(demand_left[j], supply_left[i]) + fixed_costs[i][j], i) for i in range(len(supply)) if supply_left[i] > 0]
                    if len(col_costs) > 1:
            ...
            if axis == 'row':
                min_cost, min_j = min((costs[idx][j] * min(demand_left[j], supply_left[idx]) + fixed_costs[idx][j], j) for j in range(len(demand)) if demand_left[j] > 0)
                ...
                min_cost, min_i = min((costs[i][idx] * min(demand_left[idx], supply_left[i]) + fixed_costs[i][idx], i) for i in range(len(supply)) if supply_left[i] > 0)
...
def process_all_instances_with_fixed_cost():
    ...
    for method, result_data in results.items():
        if result_data:
            ...
            output_file = f"Lab_FCR_solved_{method.upper()}_TCC.xlsx"
